...
import { Shield, FileUp, Loader2, AlertCircle, ArrowLeft, Plus, Trash2, Calculator, Sparkles, ExternalLink } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0f1e] flex flex-col items-center justify-center p-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-100 mt-8 mb-2">Tax Mitra is Thinking</h2>
        <p className="text-slate-400 text-sm max-w-xs">{loadingStep || 'Processing your statement...'}</p>
      </div>
    );
  }
        ...
        <header className="sticky top-0 z-50 border-b border-[#1e293b] bg-[#0a0f1e]/80 backdrop-blur-xl">
          <div className="max-w-7xl mx-auto flex items-center justify-between px-6 h-16">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold tracking-tight text-slate-100">Tax<span className="text-emerald-400">Mitra</span></span>
              <span className="hidden sm:inline text-slate-600 mx-2">|</span>
              <span className="hidden sm:inline text-sm text-slate-500 font-medium">Capital Gains Analysis</span>
            </div>
            <button 
    ...
    <div className="min-h-screen bg-[#0a0f1e] text-slate-200">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 animate-fade-in">
        <div className="mb-12 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/[0.08] border border-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-widest mb-4">
            Phase 2: Tax Mitra
          </div>
          <h1 className="text-4xl md:text-6xl font-extrabold text-slate-100 tracking-tight mb-4">Indian Capital Gains</h1>
        ...
        {mode === 'selection' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto mt-8">
            <button 
              ...
              className="group bg-[#111827] border border-[#1e293b] rounded-2xl p-8 text-center hover:border-emerald-500/30 transition-all duration-300"
            >
              <div className="w-16 h-16 rounded-xl bg-emerald-500/[0.08] border border-emerald-500/15 flex items-center justify-center mx-auto mb-6 group-hover:bg-emerald-500/15 transition-colors">
                <FileUp className="w-8 h-8 text-emerald-400" />
              </div>
              <h3 className="text-xl font-bold text-slate-100 mb-2">Upload CAMS PDF</h3>
              <p className="text-slate-500 text-sm leading-relaxed">Fast and automatic. Your PDF is processed in memory and never stored.</p>
            </button>
            ...
            >
              ...
              <p className="text-slate-500 text-sm leading-relaxed">Enter transactions manually. Best for calculating specific redemption scenarios.</p>
            </button>
          </div>
        )}
        ...
        {mode === 'upload' && (
          <div className="max-w-2xl mx-auto">
            <button onClick={() => setMode('selection')} className="mb-6 flex items-center gap-2 text-slate-500 hover:text-slate-300 transition-colors text-sm font-medium">
              <ArrowLeft className="w-4 h-4" /> Back to Selection
            </button>
            <Card className={`border-2 border-dashed transition-all p-12 text-center ${file ? 'border-emerald-500/40 bg-emerald-500/[0.02]' : 'border-[#1e293b] hover:border-slate-700'}`}>
              ...
              <label htmlFor="cams-upload" className="cursor-pointer block">
                <div className="w-20 h-20 rounded-2xl bg-[#0a0f1e] border border-[#1e293b] flex items-center justify-center mx-auto mb-6 shadow-xl">
                  <FileUp className={`w-10 h-10 ${file ? 'text-emerald-400' : 'text-slate-600'}`} />
                </div>
                {file ? (
                  <div>
                    <h3 className="text-xl font-bold text-slate-100 mb-1">{file.name}</h3>
                    <p className="text-slate-500 text-sm data-value">{(file.size / 1024 / 1024).toFixed(2)} MB • Ready</p>
                  </div>
                ) : (
                  <div>
                    <h3 className="text-xl font-bold text-slate-200 mb-2">Drop your CAMS PDF here</h3>
                    <p className="text-slate-500 text-sm">or click to browse your local files</p>
                  </div>
                )}
              </label>
            </Card>
            ...
            {error && (
              <div className="mt-6 p-4 bg-red-500/[0.08] border border-red-500/20 rounded-xl flex gap-3 text-red-300 text-xs leading-relaxed">
            ...
            <div className="space-y-6">
                ...
                <Card key={fIndex} className="p-6 md:p-8 animate-fade-in">
                  <div className="flex justify-between items-center mb-8">
                    <input 
                      ...
                      className="bg-transparent text-xl md:text-2xl font-bold border-b border-[#1e293b] focus:border-emerald-500/50 outline-none w-3/4 pb-2 transition-all text-slate-100 placeholder:text-slate-700"
                    ...
                    />
                    <button onClick={() => setManualFolios(manualFolios.filter((_, i) => i !== fIndex))} className="text-slate-600 hover:text-red-400 p-2 transition-colors">
                  ...
                  <div className="space-y-4">
                    {folio.transactions.map((tx: any, tIndex: number) => (
                      <div key={tIndex} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end bg-[#0a0f1e] p-4 rounded-xl border border-[#1e293b]">
                        <div>
                          <label className="section-label block mb-1.5 px-1">Date</label>
                          <input type="date" className="bg-[#111827] border border-[#1e293b] rounded-lg p-2.5 w-full text-xs text-slate-200" value={tx.date} onChange={(e) => {
                            const newFolios = [...manualFolios];
                        ...
                        <div>
                          ...
                          <select className="bg-[#111827] border border-[#1e293b] rounded-lg p-2.5 w-full text-xs text-slate-200" value={tx.transaction_type} onChange={(e) => {
                            const newFolios = [...manualFolios];
                        ...
                        <div>
                          ...
                          <input type="number" placeholder="0.000" className="bg-[#111827] border border-[#1e293b] rounded-lg p-2.5 w-full text-xs text-slate-200 data-value" value={tx.units || ''} onChange={(e) => {
                            const newFolios = [...manualFolios];
                        ...
                        <div>
                          ...
                          <input type="number" placeholder="0.00" className="bg-[#111827] border border-[#1e293b] rounded-lg p-2.5 w-full text-xs text-slate-200 data-value" value={tx.nav || ''} onChange={(e) => {
                            const newFolios = [...manualFolios];
                    ...
                    <button 
                      ...
                      className="w-full py-3 border border-dashed border-[#1e293b] rounded-xl text-slate-500 text-[10px] font-bold uppercase tracking-wider hover:border-emerald-500/30 hover:text-emerald-400 transition-all flex items-center justify-center gap-2"
                    >
              ...
              <div className="flex flex-col gap-4 mt-8">
                <button 
                  ...
                  className="w-full py-4 border-2 border-dashed border-emerald-500/10 rounded-2xl text-emerald-500/40 font-bold hover:border-emerald-500/30 hover:text-emerald-400 transition-all flex items-center justify-center gap-2"
                >
